...
source_feature_version <- "v1_basic"
target_feature_version <- "v2_enhanced"
...
", schema_name, source_feature_version)
...
if (nrow(team_pre_df) == 0) {
  stop(sprintf(
    "No rows in %s.team_pre_match_features for feature_version = '%s'",
    schema_name, source_feature_version
  ))
}
...
# =========================
# 5) build enhanced pair-level features
# =========================
pair_features <- pair_df %>%
  transmute(
    ...
    # -------- new bilateral raw features --------
    home_last5_points_avg = home_last5_points_avg,
    away_last5_points_avg = away_last5_points_avg,
    home_last10_points_avg = home_last10_points_avg,
    away_last10_points_avg = away_last10_points_avg,

    home_last5_goal_diff_avg = home_last5_goal_diff_avg,
    away_last5_goal_diff_avg = away_last5_goal_diff_avg,
    home_last10_goal_diff_avg = home_last10_goal_diff_avg,
    away_last10_goal_diff_avg = away_last10_goal_diff_avg,

    home_clean_sheet_rate_10 = home_clean_sheet_rate_10,
    away_clean_sheet_rate_10 = away_clean_sheet_rate_10,
    home_scoring_rate_10 = home_scoring_rate_10,
    away_scoring_rate_10 = away_scoring_rate_10,

    home_days_since_last_match = home_days_since_last_match,
    away_days_since_last_match = away_days_since_last_match,

    # -------- new absolute gap features --------
    abs_last5_points_diff = ifelse(
      is.na(home_last5_points_avg) | is.na(away_last5_points_avg),
      NA_real_,
      abs(as.numeric(home_last5_points_avg - away_last5_points_avg))
    ),

    abs_last10_points_diff = ifelse(
      is.na(home_last10_points_avg) | is.na(away_last10_points_avg),
      NA_real_,
      abs(as.numeric(home_last10_points_avg - away_last10_points_avg))
    ),

    abs_last5_goal_diff_diff = ifelse(
      is.na(home_last5_goal_diff_avg) | is.na(away_last5_goal_diff_avg),
      NA_real_,
      abs(as.numeric(home_last5_goal_diff_avg - away_last5_goal_diff_avg))
    ),

    abs_last10_goal_diff_diff = ifelse(
      is.na(home_last10_goal_diff_avg) | is.na(away_last10_goal_diff_avg),
      NA_real_,
      abs(as.numeric(home_last10_goal_diff_avg - away_last10_goal_diff_avg))
    ),

    abs_rest_days_diff_home_away = ifelse(
      is.na(home_days_since_last_match) | is.na(away_days_since_last_match),
      NA_integer_,
      abs(as.integer(home_days_since_last_match - away_days_since_last_match))
    ),
    ...
    friendly_flag = case_when(
      !is.na(home_friendly_flag) ~ home_friendly_flag,
      !is.na(away_friendly_flag) ~ away_friendly_flag,
      TRUE ~ FALSE
    ),

    tournament_match_flag = case_when(
      !is.na(home_tournament_match_flag) ~ home_tournament_match_flag,
      !is.na(away_tournament_match_flag) ~ away_tournament_match_flag,
      TRUE ~ FALSE
    ),
...
message("Enhanced pair feature rows built: ", nrow(pair_features))

# =========================
# 6) remove existing rows of same version
# =========================
existing_sql <- sprintf("
  SELECT match_id, feature_version
  FROM %s.team_pair_pre_match_features
  WHERE feature_version = '%s'
", schema_name, target_feature_version)
